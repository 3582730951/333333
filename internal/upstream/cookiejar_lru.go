...
func newJarLRU(max int) *jarLRU {
	...
	return &jarLRU{max: max, ll: list.New(), items: make(map[string]*list.Element)}
}
...
func (l *jarLRU) getOrCreate(key string) *cookiejar.Jar {
	...
	if el, ok := l.items[key]; ok {
		l.ll.MoveToFront(el)
		return el.Value.(*jarEntry).jar
	}
	jar, _ := cookiejar.New(nil)
	el := l.ll.PushFront(&jarEntry{key: key, jar: jar})
	l.items[key] = el
	if l.ll.Len() > l.max {
