...
func (s *Server) writeDiagnosticsExport(ctx context.Context, dst io.Writer, snapshotID string, snapshotStore *storage.Store) error {
	...
	summary["compatibility_manifest"] = s.compatibilityManifestStatus()
	if s.passiveHealth != nil {
		summary["passive_provider_health"] = s.passiveHealth.Snapshot()
	}
	...
	addCSV("http_requests.csv", []string{"request_id", "method", "route", "status", "request_bytes", "response_bytes", "duration_ms", "time_to_first_byte_ms", "stream_duration_ms", "streaming", "created_at"}, httpRequestRows(httpRequests))
	...
	passiveHealth := interface{}(map[string]interface{}{"series_count": 0})
	if s.passiveHealth != nil {
		passiveHealth = s.passiveHealth.Snapshot()
	}
	if err := addJSON("passive_provider_health.json", passiveHealth); err != nil {
		return err
	}
...
func buildDiagnosticsZipFiles(accounts []storage.Account, tokensByID map[string]storage.AccountToken, auditRows []storage.AuditLogRow, cfRows []storage.CFEvent, usageRows []diagnosticUsageRecord, holds []diagnosticBillingHold, bindings []storage.AccountEgressBinding, egressProfiles []storage.EgressProfile, capabilities []storage.ModelCapability, kiroRuntimeCapabilities []storage.KiroRuntimeCapability, rateLimits []storage.AccountRateLimit, affinityBindings []storage.AffinityBinding, settings []diagnosticSetting, customProviders []storage.CustomProvider, upstreamRules []storage.UpstreamErrorRule, resetConsumptions []storage.CodexResetCreditConsumption, lifecycleStatuses []diagnosticLifecycleStatus, reauthConfigs []storage.AccountCodexReauthConfig, reauthJobs []storage.AccountCodexReauthJob, codebook diagnosticCodebook) (map[string]string, error) {
	...
	addCSV("http_requests.csv", []string{"request_id", "method", "route", "status", "request_bytes", "response_bytes", "duration_ms", "time_to_first_byte_ms", "stream_duration_ms", "streaming", "created_at"}, nil)
	addCSV("route_attempts.csv", []string{"request_id", "tier", "target", "selection_type", "status_class", "fallback_target", "terminal_error_class", "effective_status", "super_instruct_client_choice", "super_instruct_effective_modules", "user_group_alias", "created_at"}, nil)
	...
	if err := addJSON("passive_provider_health.json", map[string]interface{}{
		"series_count": 0, "series": []interface{}{}, "source": "legacy_in_memory_export",
	}); err != nil {
		return nil, err
	}
	rowCounts["manifest.json"] = 1
