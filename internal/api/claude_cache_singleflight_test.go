...
func TestClaudeCacheSingleflightShortBodyIgnoresOnlyBillingNonce(t *testing.T) {
	...
	first := claudeCacheSingleflightKey(build("a5e", "same request"), affinity)
	second := claudeCacheSingleflightKey(build("2e4", "same request"), affinity)
	if first == "" || first != second {
		t.Fatalf("rotating billing suffix split one exact request: %q vs %q", first, second)
	}
	changedContext := claudeCacheSingleflightKey(build("a5e", "different request"), affinity)
