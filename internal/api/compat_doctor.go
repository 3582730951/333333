	...
	writeJSON(w, http.StatusOK, map[string]interface{}{
		...
		"checks": map[string]interface{}{
			...
			"claude_runtime_mode": map[string]interface{}{
				...
				"codex_model_setup":  "gateway init --pool-url <pool> --key <key> --model <gpt-or-codex-model>",
				"required_model_env": "ANTHROPIC_MODEL + ANTHROPIC_DEFAULT_HAIKU/SONNET/OPUS/FABLE_MODEL + CLAUDE_CODE_SUBAGENT_MODEL",
				"detail":             "compat keeps official Claude Code skills/plugins/MCP visible; when using a Codex model, the gateway pins every Claude tier and sub-agent role to that model so Agent/Workflow children do not fall back to claude-* requests",
			},
		},
