...
func codexCLIVersionHelp() string {
	return fmt.Sprintf(
		"留空=最新 %s；自动分析下游版本并匹配 %s 指纹。",
		config.DefaultClientVersion,
		strings.Join(config.SupportedCodexCLIVersions(), " / "),
	)
}
...
func configFields() []configField {
	return []configField{
		...
		{Key: "codex_cli_version", Label: "Codex CLI 版本覆盖", Category: catIdentity, Type: fieldString, Effect: effectUpstream,
			Help: codexCLIVersionHelp(), boot: func(c config.Config) interface{} { return c.CodexCLIVersionOverride }},
		...
		{Key: "identity_convergence_mode", Label: "设备身份收敛", Category: catIdentity, Type: fieldSelect, Effect: effectUpstream,
			Options: []string{"account", "off", "full"},
			Help:    "account(默认)=每个账号一台虚拟设备，跨出口IP保持不变，上游看到的是“一号一机”；off=账号+出口各自独立设备(换IP即新增一台设备，会累积成一号多机)；full=本部署内所有账号收敛到同一台设备。仅影响设备指纹，不合并原生 Session/Thread；既有会话保留已持久化身份。", boot: func(c config.Config) interface{} { return firstNonEmpty(c.IdentityConvergenceMode, "account") }},
		...
		{Key: "codex_429_guard_runtime_enabled", Label: "Codex 429 Guard 运行时开关", Category: catLimits, Type: fieldBool, Effect: effectHot,
			Help: "默认关闭。开启后，所有符合条件的 Codex/OpenAI OAuth 或 SetupToken 账号启用两次明确 429 确认与冷却；账号级 Guard 仅额外追加合成工具 checkpoint，且不绕过 quota 或 cooldown。", boot: func(c config.Config) interface{} { return c.Codex429GuardRuntimeEnabled }},
		{Key: "codex_429_guard_fallback_cooldown_seconds", Label: "Codex 429 Guard 回退冷却（秒）", Category: catLimits, Type: fieldInt, Effect: effectHot,
			Help: "两次明确 429 但无可信 reset 时的合成短冷却；默认 5，范围 1–7200，设为 0 则关闭该回退但仍尊重真实 quota reset。", boot: func(c config.Config) interface{} { return c.Codex429GuardFallbackCooldownSeconds }},
		{Key: "gpt_session_rollover_runtime_enabled", Label: "GPT 会话轮换运行开关", Category: catLimits, Type: fieldBool, Effect: effectHot,
			Help: "默认关闭。仅作为运维止损开关；不会单独授权任何用户组，必须同时启用对应用户组策略。", boot: func(c config.Config) interface{} { return c.GPTSessionRolloverRuntimeEnabled }},
