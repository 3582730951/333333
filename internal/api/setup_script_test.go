...
func TestSetupScriptCanForceCodexWebSocketsWhenIngressIsKnownGood(t *testing.T) {
	if _, err := exec.LookPath("bash"); err != nil {
		t.Skip("bash not available")
	}
	home := t.TempDir()
	scriptPath := filepath.Join(t.TempDir(), "setup.sh")
	if err := os.WriteFile(scriptPath, []byte(buildCodexConfigScript("https://pool.example/", "cap_abc123", "gpt-5.5", "", "", "")), 0o700); err != nil {
		t.Fatal(err)
	}
	cmd := exec.Command("bash", scriptPath)
	cmd.Env = []string{
		...
		"POOL_CLIENT=codex",
		"POOL_INSTALL_RTK=0",
		"POOL_CODEX_WEBSOCKETS=1",
	}
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("forced WebSocket setup failed: %v\n%s", err, out)
	}
	config, err := os.ReadFile(filepath.Join(home, ".codex", "config.toml"))
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(config), "supports_websockets = true") {
		t.Fatalf("forced WebSocket config missing:\n%s", config)
	}
}

func TestSetupScriptAutoDetectsPublicWebSocketUpgrade(t *testing.T) {
	if _, err := exec.LookPath("bash"); err != nil {
		t.Skip("bash not available")
	}
	...
	home := t.TempDir()
	scriptPath := filepath.Join(t.TempDir(), "setup.sh")
	if err := os.WriteFile(scriptPath, []byte(buildCodexConfigScript(ingress.URL, "cap_abc123", "gpt-5.5", "", "", "")), 0o700); err != nil {
		t.Fatal(err)
	}
	cmd := exec.Command("bash", scriptPath)
	cmd.Env = []string{
		...
		"POOL_CLIENT=codex",
		"POOL_INSTALL_RTK=0",
		"POOL_CODEX_WEBSOCKETS=auto",
		"POOL_CODEX_WS_PROBE_TIMEOUT_SECONDS=1",
	}
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("automatic WebSocket setup failed: %v\n%s", err, out)
	}
	config, err := os.ReadFile(filepath.Join(home, ".codex", "config.toml"))
	if err != nil {
		t.Fatal(err)
	...
	if !strings.Contains(string(config), "supports_websockets = true") {
		t.Fatalf("automatic WebSocket probe did not enable a working ingress:\n%s", config)
	}
}
