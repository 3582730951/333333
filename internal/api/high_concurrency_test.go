...
func testGatewayDirectConcurrentStreams(t *testing.T, count int, timeout time.Duration) {
	...
	if count > 512 {
		// The 1000-stream case exercises the 512-connection tier and its queue.
		// The default memory tier now admits 256 upstream connections, which
		// cannot reach this test's 512-arrival barrier before releasing streams.
		cfg := h.app.cfg
		cfg.BodyMemoryBudgetBytes = 512 << 20
		h.closeRuntime()
		h.serve(NewServer(Dependencies{
			Config: cfg, Store: h.store, Scheduler: scheduler.New(h.store, cfg),
			Upstream: upstream.NewClient(cfg), Planner: virtual.NewPlanner(h.store, cfg),
		}))
	}
