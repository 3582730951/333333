...
//   - modelInfo: Model information from registry (nil means no thinking support)
...
//   - ThinkingError if validation fails
...
func convertAutoToMidRange(config ThinkingConfig, support *registry.ThinkingSupport, provider, model string) ThinkingConfig {
	// For level-only models, use ModeLevel with medium
...
func isSameProviderFamily(from, to string) bool {
	...
	// Currently only Claude and Codex, so simple comparison
	return false
}
