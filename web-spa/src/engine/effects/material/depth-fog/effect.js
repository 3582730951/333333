...
export const manifest = Object.freeze({
  ...
  id: 'depth-fog',
  title: 'Depth Fog',
  composition: Object.freeze({ slot: 'foreground', blend: 'alpha', zIndex: 38, priority: 38, exclusiveGroup: 'material-surface' }),
  uniforms: Object.freeze({
    ...
    uTint: Object.freeze({ type: 'vec3', default: [0.3, 0.56, 0.7], description: 'Fog tint.' }),
    uIntensity: Object.freeze({ type: 'float', default: 0.16, min: 0, max: 0.5, step: 0.01, description: 'Haze opacity.' }),
    uDensity: Object.freeze({ type: 'float', default: 0.9, min: 0.2, max: 3, step: 0.01, description: 'Wisp density.' }),
    uFalloff: Object.freeze({ type: 'float', default: 1.7, min: 0.3, max: 4, step: 0.01, description: 'Depth falloff exponent.' }),
    uSpeed: Object.freeze({ type: 'float', default: 0.16, min: -1, max: 1, step: 0.01, description: 'Fixed-clock haze drift.' }),
    uDepth: Object.freeze({ type: 'float', default: 0.55, min: 0, max: 1, step: 0.01, description: 'Perceived panel depth.' }),
    uCornerRadius: Object.freeze({ type: 'float', default: 0.08, min: 0, max: 0.3, step: 0.01, description: 'Rounded panel mask.' }),
  }),
  quality: Object.freeze({
    high: Object.freeze({ detail: 0.8, alphaCap: 1 }),
    medium: Object.freeze({ detail: 0.5, alphaCap: 0.72 }),
    low: Object.freeze({ detail: 0.18, alphaCap: 0.46 }),
  }),
  cost: Object.freeze({
    budgetUnits: Object.freeze({ high: 0.32, medium: 0.2, low: 0.08 }),
    gpuMilliseconds: Object.freeze({ high: 0.09, medium: 0.058, low: 0.03 }),
    fill: 'partial',
...
export function createEffect(context, initialParameters = {}) {
  ...
  let targetIntensity = bounded(initialParameters.uIntensity, definitions.uIntensity); let currentIntensity = targetIntensity; let targetDensity = bounded(initialParameters.uDensity, definitions.uDensity); let currentDensity = targetDensity; let targetFalloff = bounded(initialParameters.uFalloff, definitions.uFalloff); let currentFalloff = targetFalloff; let targetSpeed = bounded(initialParameters.uSpeed, definitions.uSpeed); let currentSpeed = targetSpeed; let targetDepth = bounded(initialParameters.uDepth, definitions.uDepth); let currentDepth = targetDepth; let targetCornerRadius = bounded(initialParameters.uCornerRadius, definitions.uCornerRadius); let currentCornerRadius = targetCornerRadius;
  let profile = manifest.quality.medium; let active = true; let disposed = false;
  function setParameters(next = {}) {
    if (Object.prototype.hasOwnProperty.call(next, 'uElementRect')) targetRect = rect(next.uElementRect, targetRect); if (Object.prototype.hasOwnProperty.call(next, 'uTint')) targetTint = tint(next.uTint, targetTint); if (Object.prototype.hasOwnProperty.call(next, 'uIntensity')) targetIntensity = bounded(next.uIntensity, definitions.uIntensity); if (Object.prototype.hasOwnProperty.call(next, 'uDensity')) targetDensity = bounded(next.uDensity, definitions.uDensity);
    if (Object.prototype.hasOwnProperty.call(next, 'uFalloff')) targetFalloff = bounded(next.uFalloff, definitions.uFalloff); if (Object.prototype.hasOwnProperty.call(next, 'uSpeed')) targetSpeed = bounded(next.uSpeed, definitions.uSpeed); if (Object.prototype.hasOwnProperty.call(next, 'uDepth')) targetDepth = bounded(next.uDepth, definitions.uDepth); if (Object.prototype.hasOwnProperty.call(next, 'uCornerRadius')) targetCornerRadius = bounded(next.uCornerRadius, definitions.uCornerRadius);
  }
...
export function applyDomFallback(root, detail = {}) {
  ...
  const attribute = 'data-aurora-depth-fog-fallback'; const previous = root.getAttribute(attribute); root.setAttribute(attribute, detail.state || 'active'); let cleaned = false;
