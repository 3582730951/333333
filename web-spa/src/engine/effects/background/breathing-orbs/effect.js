...
export const manifest = Object.freeze({
  ...
  composition: Object.freeze({ slot: 'ambient', blend: 'alpha', zIndex: 18, priority: 18, exclusiveGroup: 'background-ambient' }),
  uniforms: Object.freeze({
    uIntensity: Object.freeze({ type: 'float', default: 0.21, min: 0, max: 0.48, step: 0.01, description: 'Orb opacity.' }),
    uSpeed: Object.freeze({ type: 'float', default: 0.3, min: 0, max: 1.3, step: 0.01, description: 'Breathing cycle speed.' }),
    uScale: Object.freeze({ type: 'float', default: 0.92, min: 0.45, max: 1.7, step: 0.01, description: 'Orb field scale.' }),
    uBreath: Object.freeze({ type: 'float', default: 0.5, min: 0, max: 1, step: 0.01, description: 'Pulse amplitude.' }),
  }),
  quality: Object.freeze({
    high: Object.freeze({ renderScale: 1, orbCount: 4, alphaCap: 1 }),
    medium: Object.freeze({ renderScale: 0.75, orbCount: 3, alphaCap: 0.8 }),
    low: Object.freeze({ renderScale: 0.5, orbCount: 2, alphaCap: 0.58 }),
  }),
  cost: Object.freeze({
    budgetUnits: Object.freeze({ high: 2.1, medium: 1.25, low: 0.54 }),
    gpuMilliseconds: Object.freeze({ high: 0.7, medium: 0.41, low: 0.22 }),
...
export function createEffect(context, initialParameters = {}) {
  ...
  function render(frame) {
    ...
    for (let index = 0; index < names.length; index += 1) {
      ...
      if (name === 'uScale') value *= 0.82 + profile.alphaCap * 0.18;
...
export function applyDomFallback(root) {
  ...
  const attribute = 'data-aurora-breathing-orbs-fallback';
