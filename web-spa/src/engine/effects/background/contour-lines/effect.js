...
export const manifest = Object.freeze({
  ...
  id: 'contour-lines',
  title: 'Contour Lines',
  composition: Object.freeze({ slot: 'ambient', blend: 'alpha', zIndex: 19, priority: 19, exclusiveGroup: 'background-ambient' }),
  uniforms: Object.freeze({
    uIntensity: Object.freeze({ type: 'float', default: 0.16, min: 0, max: 0.42, step: 0.01, description: 'Topographic line opacity.' }),
    uSpeed: Object.freeze({ type: 'float', default: 0.12, min: 0, max: 0.8, step: 0.01, description: 'Fixed-clock contour drift.' }),
    uScale: Object.freeze({ type: 'float', default: 1.12, min: 0.5, max: 2.4, step: 0.01, description: 'Contour map scale.' }),
    uSpacing: Object.freeze({ type: 'float', default: 0.12, min: 0.04, max: 0.3, step: 0.005, description: 'Contour interval.' }),
  }),
  quality: Object.freeze({
    ...
    low: Object.freeze({ renderScale: 0.5, detailScale: 0.52, alphaCap: 0.58 }),
  }),
  cost: Object.freeze({
    budgetUnits: Object.freeze({ high: 2.4, medium: 1.5, low: 0.65 }),
    gpuMilliseconds: Object.freeze({ high: 0.95, medium: 0.57, low: 0.3 }),
...
export function createEffect(context, initialParameters = {}) {
  ...
  function render(frame) {
    ...
    for (let index = 0; index < names.length; index += 1) {
      ...
      if (name === 'uScale') value *= profile.detailScale;
...
export function applyDomFallback(root) {
  ...
  const attribute = 'data-aurora-contour-lines-fallback';
