...
async function warmDashboard(client: QueryClient) {
...
async function warmAccounts(client: QueryClient) {
...
async function warmGroups(client: QueryClient, cancelled: Cancelled) {
...
async function warmQuota(client: QueryClient) {
...
async function warmUsage(client: QueryClient) {
...
export async function warmAdminRouteData(client: QueryClient, path: string, cancelled: Cancelled = () => false) {
  switch (path.split('?', 1)[0]) {
    case '/': await warmDashboard(client); break;
    case '/accounts': await warmAccounts(client); break;
    case '/groups': await warmGroups(client, cancelled); break;
    case '/quota': await warmQuota(client); break;
    case '/usage': await warmUsage(client); break;
    default: break;
  }
}
...
export async function warmAdminData(client: QueryClient, cancelled: Cancelled = () => false) {
  const tasks = ['/', '/accounts', '/groups', '/quota', '/usage']
    .map((path) => () => warmAdminRouteData(client, path, cancelled));
