...
async function mockBackend(page: Page, role: Role, state: FixtureState = 'ready') {
  await page.route('**/*', async (route: Route) => {
    ...
    if (url.pathname === '/client/performance') return route.fulfill({ status: 204, body: '' });
    ...
    // EventSource requires an SSE response. Returning the default JSON fixture
    // here makes every page with the ambient event stream emit a browser error.
...
test('Apple responsive controls retain focus, touch targets, tablet labels, and balanced sheets', async ({ browser }) => {
  ...
  expect(menuMetrics.outlineStyle).not.toBe('none');
  expect(menuMetrics.outlineWidth).toBeGreaterThanOrEqual(2);
  ...
  expect(labels).toHaveLength(portalVisualRoutes.length);
...
test('access and audit pages switch locale without remounting', async ({ browser }) => {
  ...
  await expect(adminPage.getByText('Network throughput', { exact: true })).toBeVisible();
