...
const settingsFile = path.join(root, 'src', 'pages', 'SettingsV2.tsx');
...
if (!source.includes("from '../components/SettingsTabShell.jsx'")) {
  problems.push('SettingsV2.tsx must use the shared SettingsTabShell wrapper.');
}

if (/loading\s*&&\s*!lastRefresh/.test(source)) {
  problems.push('SettingsV2.tsx must not duplicate first-load spinner checks inside each tab.');
}

if (/\bSavedDiffPanel\b/.test(source) || /\bSettingsErrorBanner\b/.test(source)) {
  problems.push('SettingsV2.tsx must keep diff and settings-error rendering in SettingsTabShell.');
}

if (/onUndo=\{\s*\(\)\s*=>\s*\{\s*\}\s*\}/.test(source)) {
  problems.push('SettingsV2.tsx must not render a fake undo action for tabs without undo support.');
}

if (!/\bfunction\s+ConfigFieldRow\b/.test(source)) {
  problems.push('SettingsV2.tsx must keep config field rows in the ConfigFieldRow component.');
}

if (/className="pool-settings-row"\s+style=/.test(source)) {
  problems.push('SettingsV2.tsx must keep config row layout in shared styles, not inline JSX styles.');
}

if (!/\bfunction\s+settingsFormKey\b/.test(source)) {
  problems.push('SettingsV2.tsx must use settingsFormKey for async Form initValues remount keys.');
}
...
traverse(ast, {
  JSXOpeningElement(pathRef) {
    ...
    problems.push(`SettingsV2.tsx:${loc.line}:${loc.column + 1} Form with async initValues must use key={settingsFormKey(...)}.`);
  },
});
