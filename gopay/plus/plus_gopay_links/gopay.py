...
_GOPAY_SRC = Path(__file__).resolve().parents[1] / "gopay" / "app" / "src"
if str(_GOPAY_SRC) not in sys.path:
    sys.path.insert(0, str(_GOPAY_SRC))
from opai.core.snap_signature import sign_snap_request, snap_json_body
...
# Runtime keys are loaded from config/env so the public repository does not
# carry reusable payment/client identifiers.
DEFAULT_MIDTRANS_CLIENT_ID = os.environ.get("MIDTRANS_CLIENT_KEY", "")
...
def _require_runtime_value(name: str, value: str) -> str:
    if not value or value.startswith("CHANGE_ME"):
        raise GoPayError(f"missing required runtime config: {name}")
    return value
...
class GoPayCharger:
    ...
    def __init__(
        ...
        self.midtrans_client_id = _require_runtime_value(
            "gopay.midtrans_client_id or MIDTRANS_CLIENT_KEY",
            str(gopay_cfg.get("midtrans_client_id") or DEFAULT_MIDTRANS_CLIENT_ID),
        )
    ...
    @staticmethod
    def _expected_amount_from_init(init_data: dict) -> str:
        invoice = init_data.get("invoice") or {}
        for key in ("amount_due", "total", "subtotal"):
            val = invoice.get(key)
            if val is not None:
                return str(int(val))
        summary = init_data.get("total_summary") or {}
        for key in ("due", "total", "subtotal"):
            val = summary.get(key)
            if val is not None:
                return str(int(val))
        return "0"
    ...
    def _stripe_confirm(self, cs_id: str, pm_id: str, stripe_pk: str) -> dict:
        ...
        expected_amount = self._expected_amount_from_init(init_data)
        self.log(f"[gopay] stripe confirm expected_amount={expected_amount}")
    ...
    def _midtrans_load_transaction(self, snap_token: str):
        ...
        r = self.ext.get(
            ...
            headers=self._midtrans_headers(
                snap_token,
                source=True,
                snap_path=f"/snap/v1/transactions/{snap_token}",
            ),
            timeout=DEFAULT_TIMEOUT,
        )
        r.raise_for_status()
        ...
        try:
            self.ext.post(
                ...
                headers=self._midtrans_headers(
                    ...
                    source=True,
                    origin=True,
                    snap_path=f"/snap/v1/promos/{snap_token}/search",
                ),
                timeout=DEFAULT_TIMEOUT,
            )
        except Exception as e:
        ...
        try:
            self.ext.get(
                ...
                headers=self._midtrans_headers(
                    snap_token,
                    source=True,
                    snap_path=f"/snap/v3/experiment?id={snap_token}",
                ),
                timeout=DEFAULT_TIMEOUT,
            )
        except Exception as e:
    ...
    def _midtrans_headers(
        ...
        *,
        ...
        if snap_path:
            headers.update(sign_snap_request(snap_path, snap_body))
        return headers
    ...
    def _midtrans_init_linking(self, snap_token: str) -> str:
        ...
        base_headers = self._midtrans_headers(
            snap_token, json_body=True, snap_path=link_path, snap_body=body,
        )
        auth_headers = self._midtrans_headers(
            snap_token, json_body=True, auth=True, snap_path=link_path, snap_body=body,
        )
        ...
        for _ in range(MIDTRANS_STATUS_POLL_LIMIT):
            r = self.ext.get(
                ...
                headers=self._midtrans_headers(
                    snap_token,
                    source=True,
                    snap_path=f"/snap/v1/transactions/{snap_token}/status",
                ),
                timeout=DEFAULT_TIMEOUT,
            )
            if r.status_code == 200:
...
def main():
    ...
    stripe_pk = _require_runtime_value("stripe.publishable_key or STRIPE_PUBLISHABLE_KEY", str(stripe_pk))
