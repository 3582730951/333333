...
CLIENT_SECRET = os.environ.get("GOJEK_CLIENT_SECRET", "CHANGE_ME_GOJEK_CLIENT_SECRET")
GOJEK_API_KEY = os.environ.get("GOJEK_API_KEY", "CHANGE_ME_GOJEK_API_KEY")
...
class GojekClient:
    ...
    def _format_bearer(self, token: str) -> str:
        if not token:
            return ""
        return token if token.startswith("Bearer ") else f"Bearer {token}"

    def _verification_headers(self, token: str = "") -> dict:
        """SSO endpoints after CVS verify — need Authorization + Verification-Token."""
        bearer = self._format_bearer(token or self.auth.verification_token)
        if not bearer:
            return {}
        return {"Authorization": bearer, "Verification-Token": bearer}
    ...
    def signup_create_account(
        ...
        client_auth = base64.b64encode(GOJEK_API_KEY.encode()).decode()
        xm1 = self._build_xm1()
        body_str = json.dumps(body)
        ts = str(int(time.time() * 1000))
        sig = sign_v2(
    ...
    def get_account_list(self) -> dict:
        ...
        result = self._sso_post("/goto-auth/accountlist", body, self._verification_headers())
        if result["status"] in (200, 201):
            data = result["body"].get("data", result["body"])
    ...
    def issue_token(self, grant_type: str = "cvs", token_value: str = "") -> dict:
        ...
        if grant_type not in ("refresh_token",):
            if self.auth.onefa_token:
                extra.update(self._verification_headers(self.auth.onefa_token))
            elif self.auth.verification_token:
                extra.update(self._verification_headers())
        if self.auth.access_token:
            tok = self.auth.access_token
            if not tok.startswith("Bearer "):
                tok = f"Bearer {tok}"
    ...
    def login(self, country_code: str, phone: str, pin: str, otp_callback=None) -> dict:
        ...
        # Step 5: accountlist → 1fa_token
        acct = self.get_account_list()
        if acct["status"] not in (200, 201):
            log.error(
                "accountlist failed: %d %s",
                acct["status"], acct.get("body"),
            )
            return acct
    ...
    # ========================================================================
    # Phase 5: P2P Transfer (master → child)
    #   GET  /v1/users/p2p-profile?phone_number=...  → qr_id
    #   POST /v1/funds/transfer  (461 → pin/tokens/nb → retry with pin_token)
    # ========================================================================

    @staticmethod
    def p2p_phone_lookup_formats(phone: str) -> list[str]:
        """Phone strings to try for GET /v1/users/p2p-profile (API is format-sensitive)."""
        ...
        if s.startswith("+"):
            bare = s.lstrip("+")
            if s.startswith("+86"):
                national = s[3:].lstrip("0")
                return [s, f"+86{national}", f"86{national}", national]
            if s.startswith("+62"):
                national = s[3:].lstrip("0")
                return [s, f"62{national}", f"0{national}", national]
            return [s, bare]
        digits = s.lstrip("+")
        if digits.startswith("86") and len(digits) >= 11:
            national = digits[2:].lstrip("0")
            return [f"+86{national}", f"86{national}", national]
        if digits.startswith("62"):
            national = digits[2:].lstrip("0")
            return [f"+62{national}", f"62{national}", f"0{national}", national]
        national = digits.lstrip("0")
        return [f"0{national}", f"62{national}", f"+62{national}", national]

    @staticmethod
    def normalize_p2p_phone(phone: str) -> str:
        """Best-effort local digits (legacy); prefer p2p_profile() lookup."""
        fmts = GojekClient.p2p_phone_lookup_formats(phone)
        return fmts[-1] if fmts else phone

    def p2p_profile(self, phone_number: str) -> dict:
        """GET /v1/users/p2p-profile → recipient qr_id and block status."""
        last: dict = {"status": 0, "body": {}}
        for fmt in self.p2p_phone_lookup_formats(phone_number):
            q = urllib.parse.urlencode({"phone_number": fmt})
            last = self._gopay_request("GET", f"/v1/users/p2p-profile?{q}")
            if last["status"] == 200:
                return last
        return last

    @staticmethod
    def _extract_pin_challenge(body: dict) -> tuple[str, str]:
        """Parse challenge_id + client_id from a GOPAY_PIN_CHALLENGE response."""
        if not isinstance(body, dict):
            return "", ""

        def walk(node: dict) -> tuple[str, str]:
            cid = str(node.get("challenge_id") or "")
            clid = str(node.get("client_id") or "")
            if cid:
                return cid, clid
            for key in ("data", "challenge", "action", "value", "errors"):
                nested = node.get(key)
    ...
    def pin_verify_nb(self, pin: str, challenge_id: str, client_id: str) -> dict:
        """POST /api/v1/users/pin/tokens/nb — PIN verify for transaction challenges."""
        body = {"challenge_id": challenge_id, "client_id": client_id, "pin": pin}
        return self._gopay_post(
    ...
    def transfer_funds(
        ...
        *,
        ...
        """POST /v1/funds/transfer — send GoPay balance to another user.

        GoPay returns 461 + GOPAY_PIN_CHALLENGE; exchange PIN for pin_token and retry.
        """
        body: dict = {
            "amount": {"currency": "IDR", "value": int(amount)},
            "description": description or "",
            "metadata": {
                "post_visibility": "NO_SOCIAL",
                "theme_id": "THEME_CLASSIC",
            },
            "payee": {"id": payee_qr_id, "id_type": "GOPAY_QR_ID"},
        }
        extra: dict[str, str] = {}
        if idempotency_key:
            extra["Idempotency-Key"] = idempotency_key

        first = self._gopay_post("/v1/funds/transfer", body, extra or None)
        if first["status"] in (200, 201):
            return first
        if first["status"] != 461:
            return first

        challenge_id, client_id = self._extract_pin_challenge(first.get("body") or {})
        if not challenge_id or not client_id:
            log.warning("transfer 461 but no challenge_id/client_id in body")
            return first
        log.info("transfer PIN challenge: id=%s client=%s", challenge_id[:16], client_id[-24:])

        pin_result = self.pin_verify_nb(pin, challenge_id, client_id)
        if pin_result["status"] not in (200, 201):
            log.warning("transfer pin_verify_nb → %d %s", pin_result["status"], pin_result.get("body"))
            return pin_result

        inner = (pin_result.get("body") or {}).get("data") or pin_result.get("body") or {}
        pin_token = inner.get("token", "")
        if not pin_token:
            return {"status": pin_result["status"], "body": {"error": "empty pin_token"}}

        body["challenge"] = {
            "type": "GOPAY_PIN_CHALLENGE",
            "value": {"pin_token": pin_token},
        }
        second = self._gopay_post("/v1/funds/transfer", body, extra or None)
        if second["status"] not in (200, 201):
            log.warning(
                "transfer retry after pin_token → %d (challenge_id was %s)",
                second["status"],
                challenge_id[:16],
            )
        return second
