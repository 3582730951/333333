...
from opai.core.snap_signature import midtrans_snap_headers, snap_json_body
...
class GoPayPayment:
    ...
    def _midtrans_redirection_url(self, snap: str) -> str:
        return f"{MIDTRANS_BASE}/snap/v4/redirection/{snap}"

    def _midtrans_warm_snap_side_effects(self, snap: str) -> None:
        """Replay Snap XHRs the browser fires before linking."""
        promos_path = f"/snap/v1/promos/{snap}/search"
        try:
            self._midtrans_post_empty(
                promos_path, snap, with_source=True, with_origin=True,
            )
        except Exception as exc:
            log.warning("[pay] promos warmup skipped: %s", exc)
        exp_path = f"/snap/v3/experiment?id={snap}"
        try:
            self._midtrans_get(exp_path, snap, with_source=True)
        except Exception as exc:
            log.warning("[pay] experiment warmup skipped: %s", exc)

    def _midtrans_load_transaction(self, snap: str) -> dict | None:
        """Seed Midtrans cookies/session like Snap redirection page load."""
        redirection_url = self._midtrans_redirection_url(snap)
        try:
            r = self._session.get(
                redirection_url,
                headers={
            ...
            log.info("[pay] snap redirection warmup: %d", r.status_code)
        except Exception as exc:
            log.warning("[pay] snap redirection warmup skipped: %s", exc)

        tx_path = f"/snap/v1/transactions/{snap}"
        tx = self._midtrans_get(tx_path, snap, with_source=True)
        if tx["status"] != 200:
            log.warning("[pay] transaction load failed: %d", tx["status"])
            return None
        body = tx["body"]
        ...
        if merchant_id:
            ...
            log.info("[pay] merchant=%s enabled_payments=%s", merchant_id, enabled)
        self._midtrans_warm_snap_side_effects(snap)
        return body

    def _midtrans_post_empty(
        self,
        path: str,
        snap: str,
        *,
        ...
        headers = self._midtrans_headers(path, None, snap, with_source=with_source)
        if with_origin:
            headers["Origin"] = "https://app.midtrans.com"
        r = self._session.post(url, headers=headers, data="", timeout_seconds=timeout)
        log.debug("[MT POST empty] %s → %d", path, r.status_code)
        try:
    ...
    def _midtrans_post(
        ...
        *,
        ...
        headers = self._midtrans_headers(
            path, body, snap, with_source=with_source, with_auth=with_auth,
        )
        r = self._session.post(
            url, headers=headers, data=snap_json_body(body), timeout_seconds=timeout,
        )
    ...
    def _midtrans_delete(self, path: str, snap: str, *, timeout: int = 15) -> dict:
        url = f"{MIDTRANS_BASE}{path}"
        headers = self._midtrans_headers(path, None, snap)
    ...
    def pay(
        ...
        prepare_otp: Callable[[str], None] = None,
    ) -> dict:
        """
        执行完整的 GoPay 支付流程。
        ...
        Args:
            ...
            pin: 6 位 GoPay PIN
            prepare_otp: 在 Midtrans 触发支付 OTP 前准备接码订单
        ...
        # Browser-like Snap session warmup (cookies + transaction + promos/experiment)
        log.info("[pay] Step 0: snap session warmup")
        self._midtrans_load_transaction(snap)
        time.sleep(1)
        ...
        if link_r["status"] == 406:
            ...
            ul = self._midtrans_delete(f"/snap/v3/accounts/{snap}/gopay", snap)
        ...
        if prepare_otp:
            try:
                prepare_otp(full_phone)
            except Exception as exc:
                return {"success": False, "detail": f"prepare OTP failed: {exc}"}
        ...
        for _ in range(10):
            time.sleep(2)
            gs = self._midtrans_get(f"/snap/v3/accounts/{snap}/gopay", snap)
            if gs["status"] == 200:
        ...
        ts = self._midtrans_get(f"/snap/v1/transactions/{snap}/status", snap)
