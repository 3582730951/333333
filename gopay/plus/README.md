...
完整的干净仓库发布规则、敏感文件清单和配置说明见 [`docs/CONFIGURATION.md`](docs/CONFIGURATION.md)。
...
```jsonc
{
  ...
  "orchestrator": {
    ...
    "auth_token": "CHANGE_ME_RANDOM_TOKEN"
...
curl -X POST http://localhost:8800/subscribe \
  ...
  -d '{"session_token": "YOUR_CHATGPT_SESSION_TOKEN"}'
```
...
3. curl -X POST http://localhost:8800/subscribe \
     -H "Authorization: Bearer <AUTH_TOKEN>" \
     -d '{"session_token":"YOUR_CHATGPT_SESSION_TOKEN","phone_number":"8000000000","pin":"YOUR_6_DIGIT_PIN"}'
