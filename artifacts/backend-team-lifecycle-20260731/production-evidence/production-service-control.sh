...
ROOT=/root/autodl-tmp/team-lifecycle-production-20260731
SOURCE=/root/autodl-tmp/team-lifecycle-install-20260731/prefix/bin/codex-pool-server
...
MAIN_CURRENT=/root/autodl-tmp/cpupg-20260730/prefix/lib/codex-pool/releases/apple-email-overflow-final/codex-pool-server
FRONT_CURRENT=/root/autodl-tmp/frontend-ui-shot-20260731/releases/apple-email-overflow-final/codex-pool-server
MAIN_FINAL=/root/autodl-tmp/cpupg-20260730/prefix/lib/codex-pool/releases/team-lifecycle-final/codex-pool-server
FRONT_FINAL=/root/autodl-tmp/frontend-ui-shot-20260731/releases/team-lifecycle-final/codex-pool-server

MAIN_CURRENT_RELEASE=apple-email-overflow-final-main
FRONT_CURRENT_RELEASE=apple-email-overflow-final-frontend
MAIN_FINAL_RELEASE=team-lifecycle-final-main
FRONT_FINAL_RELEASE=team-lifecycle-final-frontend
CURRENT_BINARY_SHA=429f98e8fb44b62b2fe4f71ca8745923dd6feac83629964e2ec82876e8cd9046
FINAL_BINARY_SHA=3b5a7aa6fa106414ff379e79d075ddec56afa9f53872e2688f4a9633e2fd8fd8
CURRENT_CONSOLE_SHA=c56c968e9703f066dca5e601fb836c1665ff332843512d98ea769a308ed0037f
FINAL_CONSOLE_SHA=0fb32e4bcb7a3c6a497ef3ccbf5822ae09436276603ba4a6896b3fc9e9b8b000
...
for entry in os.scandir("/proc"):
    ...
    if config in text and "--config" in text:
        print(entry.name)
PY
}
...
source, destination = sys.argv[1:3]
temp = destination + ".team-lifecycle-restore"
...
result = {
    ...
    "team_schema_tables": team_tables,
}
conn.close()
...
workflow_columns = {
    row[1] for row in conn.execute("PRAGMA table_info(team_lifecycle_workflows)")
}
member_columns = {row[1] for row in conn.execute("PRAGMA table_info(team_members)")}
forbidden = {"access_token", "refresh_token", "password", "phone_number"}
exposed = sorted((workflow_columns | member_columns) & forbidden)
result = {
    "required_tables": sorted(required),
    "missing_tables": missing,
    "forbidden_secret_columns": exposed,
}
conn.close()
open(output, "w").write(json.dumps(result, indent=2) + "\n")
assert not missing
assert not exposed
print("TEAM_SCHEMA_ASSERTED=1")
PY
}
...
prepare() {
  ...
  [[ "$(sha256sum "$MAIN_CURRENT" | awk '{print $1}')" == "$CURRENT_BINARY_SHA" ]]
  [[ "$(sha256sum "$FRONT_CURRENT" | awk '{print $1}')" == "$CURRENT_BINARY_SHA" ]]
  [[ -f "$BACKUP/main-previous.bin" ]] || cp -p "$MAIN_CURRENT" "$BACKUP/main-previous.bin"
  [[ -f "$BACKUP/frontend-previous.bin" ]] || cp -p "$FRONT_CURRENT" "$BACKUP/frontend-previous.bin"
  ...
  sha256sum "$SOURCE" "$MAIN_FINAL" "$FRONT_FINAL" >"$RECORDS/final-binaries.sha256"
}
...
verify_final() {
  ...
  verify_team_schema "$MAIN_DB" "$RECORDS/${phase}-main-team-schema.json"
  verify_team_schema "$FRONT_DB" "$RECORDS/${phase}-frontend-team-schema.json"
}
...
rollback() {
  ...
  start_service "$BACKUP/main-previous.bin" "$MAIN_CFG" "$MAIN_CURRENT_RELEASE" 34273 rollback-main
  start_service "$BACKUP/frontend-previous.bin" "$FRONT_CFG" "$FRONT_CURRENT_RELEASE" 34274 rollback-frontend
  ...
  [[ "$(console_sha 34273)" == "$CURRENT_CONSOLE_SHA" ]]
  [[ "$(console_sha 34274)" == "$CURRENT_CONSOLE_SHA" ]]
  printf 'ROLLBACK_BEHAVIOR=previous-release-ready previous-console-restored data-preserved\n'
