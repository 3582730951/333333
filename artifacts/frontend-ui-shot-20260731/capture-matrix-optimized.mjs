...
const outputDir = `${root}/screenshots/matrix-optimized`;
...
fs.writeFileSync(`${root}/artifacts/ui-matrix-optimized-report.json`, `${JSON.stringify(summary, null, 2)}\n`);
const markdown = [
  '# 云端真实 UI 截图矩阵',
  '',
  `- 截图：${summary.total}`,
...
fs.writeFileSync(`${root}/artifacts/ui-matrix-optimized-report.md`, markdown.join('\n'));
